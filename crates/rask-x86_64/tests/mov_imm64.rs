...
fn test_mov_rax_and_r10_imm64() {
    let bytes = encode(|e| {
        e.mov_reg_imm64(RAX, 1337);
        e.mov_reg_imm64(R10, 42);
    });

    // 48 b8 39 05 00 00 00 00 00 00    mov rax, 1337
    // 49 ba 2a 00 00 00 00 00 00 00    mov r10, 42
    let expected = [
        0x48, 0xB8, 0x39, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x49, 0xBA, 0x2A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
...
fn test_rex_prefix_changes_with_high_registers() {
    let bytes_low = encode(|e| e.mov_reg_imm64(RAX, 0));
    let bytes_high = encode(|e| e.mov_reg_imm64(R8, 0));
